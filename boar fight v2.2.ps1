...
# classes so far:
# Brawler (strong punch
# Agility (Strong kick)
# Homeless (strong spit)
# fat guy (strong guard)
...
Guard - does 500 damange, reduces incoming damage by 80%, 70%, or 60%
Attack choice}
...
elseif ($attack -contains "spit on")
{
...
elseif ($attack -contains "guard")
{
$boarhealth = $boarhealth - $guarddamage
write-host "
you guarded! it did $guarddamage damage!"
write-host "the boar's health is now $boarhealth"
}
...
if($attack -eq "guard")
{
$guardpercent = .50, .40, .30 | get-random
$boarattack = $boarattack*$guardpercent
}
